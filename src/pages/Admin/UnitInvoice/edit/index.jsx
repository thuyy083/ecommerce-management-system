...
function UnitInvoiceEdit() {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['unitInvoice', id],
    enabled: !!user_id && !!device_id,
    queryFn: () => unitInvoiceService.getById({ device_id, user_id, }, id),
    retry: false,
  ...
  const submit = async (data) => {
    unitInvoiceService.update({ device_id, user_id, }, id, data)
      .then(res => {
...
export default UnitInvoiceEdit
